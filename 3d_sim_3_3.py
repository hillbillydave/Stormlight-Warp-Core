#!/usr/bin/env python3
"""
3d_sim3_3_hybrid - Hybrid CPU/GPU ODIM-U warp-bubble simulation
Lower-power variant with smoother shift-vector ramp and stronger damping.
...
C       = 2.99792458e8
c_I     = 0.3 * C
beta_I  = 300.0          # stronger damping to bleed energy faster
OMEGA_0 = 2.0 * np.pi

R_bubble = 50.0
v_design = 0.03 * C      # slightly gentler design shift speed

# Bubble kinematics in grid frame (still "warp 5" in interpretation space)
bubble_v0      = 5.0 * C
bubble_a_drive = 0.0
...
ADVECT_FIELD       = True
ADVECTION_STRENGTH = 0.05   # weaker coupling to reduce energy injection

sigma_I  = 30.0
I_R_amp  = 1.0e-7           # lower amplitude -> ~25x less energy vs 5e-7
...
# Smooth ramp time for turning on the shift vector (seconds)
RAMP_TIME = 1.0e-2

VERSION = "3d_sim3_3_hybrid"
...
def ramp_envelope(t, T_ramp):
    """
    Smoothly ramps from 0 to 1 over T_ramp using a half-cosine.
    """
    if T_ramp <= 0.0:
        return 1.0
    if t >= T_ramp:
...
for n in range(Nt):
    ...
    # Smooth ramp of the shift vector to kill the initial power spike
    env = ramp_envelope(t, RAMP_TIME)
