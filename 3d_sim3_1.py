...
OMEGA_0 = 2.0 * np.pi         # 2π rad/s, Beardsley resonance
...
bubble_v0      = 5.0 * C      # 5c external-frame toy speed
bubble_a_drive = 0.0          # constant bubble acceleration (m/s^2)
...
ADVECT_FIELD       = True
ADVECTION_STRENGTH = 0.10      # reduced to soften slosh and power

# Field shape / amplitude
sigma_I  = 30.0                # width of initial I_R Gaussian (m)
I_R_amp  = 5.0e-7              # further reduced amplitude to lower power

# Grid and time parameters (grid frame)
Nx = Ny = Nz = 64              # higher resolution (was 32)
Lx = Ly = Lz = 300.0           # meters

CFL = 0.3
Nt  = 4_000_000                # longer run (~10x) for ~0.1–0.2 s physical time
...
VERSION = "v3.1_geom_FTL_5c_longrun_hiRes_lowPower"
...
# Analytic ETA to Mars at constant bubble_v0 (external frame)
if bubble_v0 > 0:
    eta_mars = D_EM / bubble_v0
    print(f"Analytic ETA to Mars at v0: {eta_mars:.3e} s (~{eta_mars/3600:.3e} hours)")
else:
    eta_mars = None
...
# Shift field N (bubble) – recomputed each step
...
# Power = d(rho_eff)/dt (instantaneous) and smoothed tail
power_arr = np.gradient(rho_eff_arr, dt)
tail_window = min(1000, len(power_arr)//10)  # last 1000 steps or last 10%
...
if eta_mars is not None:
    print(f"Analytic Mars ETA (5c): {eta_mars:.3e} s (~{eta_mars/60:.3e} min)")
